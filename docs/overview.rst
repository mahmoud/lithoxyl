...
Actions are much more than print statements. Actions are lightweight
objects that track the state of code execution, from timing
information to uncaught exceptions. Plus, each Action has a name and
a level, to enable aggregation and filtering down the line.

Actions are created with Loggers. Here's a basic example of creating
...
code succinctly actions the beginning and ending of this code
...
but it would not be very easy to create a Action without a Logger.

As we learned above, before a Action can be populated, it must be
