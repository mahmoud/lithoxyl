...
name: release
description: |
	Release lithoxyl to PyPI. Handles version bumping (CalVer YY.MINOR.MICRO),
	tagging, pushing, and post-publish verification. Use when asked to
	"release lithoxyl", "cut a release", "publish to PyPI", or "bump version".
---
...
5. Check what is actually published on PyPI: https://pypi.org/project/lithoxyl/
...
- **Failed release** (tag exists locally/on GitHub but not on PyPI): PyPI is
	...
	```
	Then check `__version__` in `lithoxyl/__init__.py`. If it was already bumped
